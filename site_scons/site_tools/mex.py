...
def generate(env):
    ...
    if not env.get("MEXC"):
        SCons.Warnings.warn(
            SCons.Warnings.DependencyWarning, "Could not find mex program."
        )
        return
    # matlab *should* be in the same directory; use it to check extension
    try:
        cmd = [
            os.path.join(os.path.split(env["MEXC"])[0], "matlab"),
            "-nodisplay",
            "-nojvm",
        ]
        ...
        bld = Builder(action="$MEXC $SOURCES -o $TARGET", suffix=env["MEX_EXT"])
        env["BUILDERS"]["MEX"] = bld
    except OSError:
        # couldn't find matlab most likely
        SCons.Warnings.warn(
            SCons.Warnings.DependencyWarning, "Could not find matlab program."
        )
